...
class DeleteController extends Controller
{
    ...
    //Magas ------------------------------------------------------------------------------------------------------------
    public function magasArticle($id)
    {
        try {
        ...
        return redirect()->route('magas.articles')->withInput()->withAlertSuccess("L'article a été effacé avec succès");
    }

    public function magasMarque($id)
    {
        try {
            DB::select("update marques set deleted=1 where id_marque=" . $id . " ");
        } catch (Exception $e) {
            return redirect()->back()->withInput()->withAlertDanger("Erreur de suppression de l'utilisateur.<br>Messagde d'erreur: <b>" . $e->getMessage() . "</b>");
        }
        return redirect()->route('magas.articles')->withInput()->withAlertSuccess("La marque a été effacée avec succès");
    }

    public function magasCategorie($id)
    {
        try {
            DB::select("update categories set deleted=1 where id_categorie=" . $id . " ");
        } catch (Exception $e) {
            return redirect()->back()->withInput()->withAlertDanger("Erreur de suppression de l'utilisateur.<br>Messagde d'erreur: <b>" . $e->getMessage() . "</b>");
        }
        return redirect()->route('magas.articles')->withInput()->withAlertSuccess("L'article a été effacé avec succès");
